...
internal class ChatConnectionTest {
    ...
    @Test
    fun `anonymous connect sends correct CAP, PASS, NICK sequence`() =
        runTest {
    ...
    @Test
    fun `authenticated connect sends correct credentials`() =
        runTest {
            withContext(Dispatchers.Default) {
                withTimeout(5.seconds) {
    ...
    @Test
    fun `connected state updates on successful handshake`() =
        runTest {
            withContext(Dispatchers.Default) {
                withTimeout(5.seconds) {
                    val conn = createConnection()
    ...
    @Test
    fun `joinChannels sends JOIN command after connect`() =
        runTest {
            withContext(Dispatchers.Default) {
                withTimeout(5.seconds) {
                    val conn = createConnection()
    ...
    @Test
    fun `partChannel sends PART command`() =
        runTest {
            withContext(Dispatchers.Default) {
                withTimeout(5.seconds) {
                    val conn = createConnection()
    ...
    @Test
    fun `sendMessage sends raw IRC through websocket`() =
        runTest {
            withContext(Dispatchers.Default) {
                withTimeout(5.seconds) {
                    val conn = createConnection()
    ...
    @Test
    fun `close resets connected state`() =
        runTest {
            withContext(Dispatchers.Default) {
                withTimeout(5.seconds) {
                    val conn = createConnection()
    ...
    @Test
    fun `PING from server is answered with PONG`() =
        runTest {
            withContext(Dispatchers.Default) {
                withTimeout(5.seconds) {
                    val conn = createConnection()
    ...
    @Test
    fun `reconnectIfNecessary does nothing when already connected`() =
        runTest {
            withContext(Dispatchers.Default) {
                withTimeout(5.seconds) {
                    val conn = createConnection()
    ...
    @Test
    fun `multiple channels are joined via single JOIN command`() =
        runTest {
            withContext(Dispatchers.Default) {
                withTimeout(5.seconds) {
                    val conn = createConnection()
